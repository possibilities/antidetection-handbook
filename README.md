...
| TLS/JA3/JA4 | Native browser emits ClientHello; ALPN agrees with subsequent protocol. | JA3 is brittle; JA4 groups more broadly; neither uniquely identifies a device. Measured: Chrome 150 shuffles ClientHello extension order on **every connection** — 10 handshakes produced 10 distinct orders of one constant 15-extension set — so JA3 changes run to run from a single unmodified browser. Baseline the extension *set*, never an order. |
